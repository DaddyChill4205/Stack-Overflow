...
def hover_over(image_name, confidence = .9):
    '''
...
def search_and_click(image_name, double = False, go_back = True, below = 0, timeout=10, confidence = .9):
...
def click_if_exists(image_name, double = False, go_back = True, below = 0, confidence = .9):
    original_position = pyautogui.position()
    found_it = pyautogui.locateOnScreen(image_name,)
    if found_it != None:
        print("found", image_name)
...
def find(image_name, confidence = .9):
    timeout = 10
    found_it = pyautogui.locateOnScreen(image_name,)
    start = time.time()
    while found_it == None:
...
def found(image_name, confidence = .9):
